...
export function usePresentation() {
  ...
  useEffect(() => {
    if (!focused) return;
    let mounted = true;
    void Promise.all([getMeta('hud.economy.v1'), getNotificationsEnabled(), getUnits(), getMeta('hud.weather.v1'), getMeta('hud.route-display.v1')])
      .then(([e, haptics, units, w, routeDisplay]) => { if (mounted) useHudPreferences.setState({ economy: e === 'true', haptics, units, weather: w !== 'false', routeDisplay: routeDisplay === 'gps' ? 'gps' : 'streets' }); })
      .catch(() => undefined);
