...
let cache: WeatherReading | null = null;
const TTL = 15 * 60_000;
...
export function useLighting(fix: RunFix | null, active: boolean, simulation: boolean) {
  ...
  const [weather, setWeather] = useState<WeatherReading | null>(cache);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      if (cache && cache.cell === cell && Date.now() - cache.fetchedAt < TTL) { setWeather(cache); return; }
      pending = true; controller = new AbortController();
      const timeout = setTimeout(() => controller?.abort(), 6000);
      try {
        const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=weather_code,cloud_cover&daily=sunrise,sunset&timezone=auto&timeformat=unixtime&forecast_days=1`, { signal: controller.signal });
        ...
        const reading: WeatherReading = { code, cloud, observedAt: observed * 1000, fetchedAt: Date.now(), cell, sunrise: typeof body.daily?.sunrise?.[0] === 'number' ? body.daily.sunrise[0] * 1000 : null, sunset: typeof body.daily?.sunset?.[0] === 'number' ? body.daily.sunset[0] * 1000 : null };
        if (!cancelled) { cache = reading; setWeather(reading); }
