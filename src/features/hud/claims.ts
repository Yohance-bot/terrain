...
export function confirmedClaimCue(result: RunResult, coordinate: [number, number], now = Date.now()): HudCue | null {
  ...
  return { id: `${result.run_id}:claim`, kind: 'claim', title: 'TERRITORY CLAIMED', detail: `${ids.length} ${ids.length === 1 ? 'territory' : 'territories'} secured`, coordinate, territoryIds: ids, createdAt: now };
}
