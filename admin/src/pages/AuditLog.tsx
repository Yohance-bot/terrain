  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">Immutable operational activity feed</p>
      </div>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr key={event.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="text-sm font-medium text-foreground">{event.actor_kind}</div>
                    <div className="text-xs text-muted-foreground font-mono mt-1">{event.actor_ref || 'system'}</div>
                  </td>
                  <td className="p-4">
                    <div className="text-sm text-foreground">{event.target_type}</div>
                    <div className="text-xs text-muted-foreground font-mono mt-1 truncate max-w-xs" title={event.target_ref}>
                      {event.target_ref}
                    </div>
                  </td>
                  <td className="p-4 text-sm text-muted-foreground max-w-md truncate" title={event.reason || ''}>
                    {event.reason || '-'}
                  </td>
                </tr>
              ))}
